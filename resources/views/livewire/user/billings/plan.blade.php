                ...
                @foreach ($products as $product)
                    @if ($product->isFresher())
                        <div @class([
                            ...
                            '!scale-105' => !auth()->user()->tenant?->hasActiveSubscription(),
                        ])>
                            <div @class([
                                ...
                                '!shadow-lg shadow-indigo-200 !bg-green-50' => !auth()->user()->tenant?->hasActiveSubscription(),
                            ])>
                    ...
                    @else
                        <div @class([
                            'flex-1 max-w-lg min-w-[320px] scale-95',
                            '!scale-105' => $product->isActive(),
                        ])>
                            <div @class([
                                'relative shadow-md z-10 mb-10 overflow-hidden rounded-[10px] border-2  border-t-4 border-t-indigo-600 dark:border-dark-3 bg-white dark:bg-dark-2 p-5',
                                '!shadow-lg shadow-indigo-200 !bg-green-50' => $product->isActive(),
                            ])>
                                <span class="block mb-3 text-lg font-semibold text-indigo-500">
                                    {{ $product->name }}
                                </span>
                                ...
                                @if ($product->isActive())
                                    <p wire:loading.inline-flex wire:target="cancel"
                                        class="inline-flex items-center gap-2 py-2 text-sm text-green-600">
                                        Please wait until processing...<span
                                            class="inline-block border-2 border-b-0 border-green-500 rounded-full size-4 animate-spin"></span>
                                    </p>
                                    <button wire:confirm="Be sure if you want to cancel it!"
                                        wire:loading.attr="disabled"
                                        wire:click="cancel({{ $product->activeSubscription->identity }})"
