<div>
    ...
    <div class="py-12">
        ...
        <div
            ...
            <div class="absolute text-lg font-bold text-right text-indigo-500 top-2 right-2">
                Marks: <span class="px-2 py-1 text-sm text-white bg-indigo-500 rounded-full">{{ $quiz->marks_total }}</span>
            </div>
    ...
    <form wire:submit="save" class="space-y-4">
        @foreach ($quiz->questions as $questionKey => $question)
            ...
            <div
                class="relative p-4 sm:p-8 bg-white border border-slate-300 border-l-4 dark:bg-gray-800 shadow sm:rounded-lg rounded-md has-[:focus]:border-l-indigo-600 space-y-3" wire:key="{{ $questionKey }}">
                ...
                @if ($question->isShortText())
                    ...
                    <input type="hidden" name="option_{{ $questionKey }}" id="option_{{ $questionKey }}" x-init="$wire.set('answers.{{ $questionKey }}.option_id.0', '{{ $question->options->first()->id }}')" wire:model="answers.{{ $questionKey }}.option_id.0" hidden class="hidden">

                    <x-question.text-input name="answer" :error="$errors?->first('answer')" placeholder="Your answer!" wire:model="answers.{{ $questionKey }}.answer.0" />
                ...
                @elseif ($question->isLongText())
                    ...
                    <x-question.textarea name="answer" :error="$errors?->first('answer')" placeholder="Your answer!" wire:model="answers.{{ $questionKey }}.answer.0" />
                    ...
                    @foreach ($question->options as $optionKey => $option)
                        <div>
                            <label>
                                ...
                                wire:target="setAnswer"
                                x-on:change="
                                    setAnswer($event.target.checked, '{{ $option->label }}')
                                    $wire.setAnswer({{ $questionKey }}, answers)
                                "
                                id="option_{{ $questionKey }}" type="radio" name="option_{{ $questionKey }}"
                                    ...
                                    wire:model="answers.{{ $questionKey }}.option_id.0" value="{{ $option->id }}">{{ $option->label }}</label>
                        </div>
                    ...
                    @foreach ($question->options as $optionKey => $option)
                        <div wire:key="{{ $optionKey }}">
                            <label>
                                ...
                                $wire.setAnswer({{ $questionKey }}, answers)
                            "
                                id="option_{{ $questionKey }}" type="checkbox" name="option_{{ $questionKey }}"
                                    class="w-4 h-4 mr-2 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 disabled:opacity-50"
                                    wire:model="answers.{{ $questionKey }}.option_id" value="{{ $option->id }}">
                                {{ $option->label }}
                            </label>
                        </div>
